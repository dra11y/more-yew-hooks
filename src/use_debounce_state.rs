use super::use_debounce::use_debounce;
use std::ops::Deref;
use std::rc::Rc;
use yew::prelude::*;
...
///
/// Fixes [`yew_hooks::use_debounce_state`], which doesn't work because it requires
/// `T: FnOnce()` (which can only be called once). We changed it to `T: Fn()`.
///
/// # Example
///
/// ```rust
